...
elseif a == 0
    ...
    s = linear_conv(t, l, m, r) * 0.1 ; 
         stem(t,s);
            else
                s = linear_conv(t, l, m, r) * 0 ;
                 stem(t,s);
            end        
    hold on
    p = p+s;
...
hold off
    ...
    if x1 == x2
            ...
            for t = -10:0.1:10
                 if t >= 0
    s = quad_equalroots_conv(t, k, l, m, r) * 0.1 ;
    stem(t,s);
                 else     
                s = quad_equalroots_conv(t, k, l, m, r) * 0 ;
                 stem(t,s);
                 end
    hold on
    p = p+s;
...
hold off
        ...
        %%roots(including coplex roots case)
            for t = -10:0.1:10
                 if t >= 0
     s = quad_unequalroots_conv(t, k, l, m, x1, x2) * 0.1 ;
    stem(t,s);
                 else
                s = quad_unequalroots_conv(t, k, l, m, x1, x2) * 0 ;
                 stem(t,s);
                 end
    hold on
    p = p+s;
